...
enum ImNodesAttributeFlags_ {
  ...
  // is connected to. NOTE: the user has to actually delete the link for this to
  ...
  // https://github.com/Nelarius/imnodes/issues/41#issuecomment-647132113 NOTE:
  // the user has to actually delete the link for this to work. A deleted link
  // can be detected by calling IsLinkDestroyed() after EndNodeEditor().
...
struct ImNodesIO {
  struct EmulateThreeButtonMouse {
    ...
    // The keyboard modifier to use in combination with mouse left click to pan
    // the editor view. Set to NULL by default. To enable this feature, set the
    // modifier to point to a boolean indicating the state of a modifier. For
    // example,
    //
  ...
  struct LinkDetachWithModifierClick {
    ...
    // pressed. Set to NULL by default. To enable the feature, set the modifier
    // to point to a boolean indicating the state of a modifier. For example,
    //
    // ImNodes::GetIO().LinkDetachWithModifierClick.Modifier =
    // &ImGui::GetIO().KeyCtrl;
    //
    // Left-clicking a link with this modifier pressed will detach that link.
  ...
  struct MultipleSelectModifier {
    ...
    // to point to a boolean indicating the state of a modifier. For example,
    //
    // ImNodes::GetIO().MultipleSelectModifier.Modifier =
    // &ImGui::GetIO().KeyCtrl;
    //
    // Left-clicking a node with this modifier pressed will add the node to the
    // list of currently selected nodes. If this value is NULL, the Ctrl key
    // will be used.
    const bool *Modifier;
  } MultipleSelectModifier;

  // Holding alt mouse button pans the node area, by default middle mouse button
  // will be used Set based on ImGuiMouseButton values
  int AltMouseButton;

  // Panning speed when dragging an element and mouse is outside the main editor
  // view.
...
struct ImNodesStyle {
  ...
  // default size of each pin shape is balanced to occupy approximately the same
  // surface area on the screen.

  // The circle radius used when the pin shape is either ImNodesPinShape_Circle
  // or ImNodesPinShape_CircleFilled.
  ...
  // The radius from the pin's center position inside of which it is detected as
  // being hovered over.
  float PinHoverRadius;
  // Offsets the pins' positions from the edge of the node to the outside of the
  // node.
  float PinOffset;
...
//
// By default, the library creates an editor context behind the scenes, so using
// any of the imnodes functions doesn't require you to explicitly create a
// context.
struct ImNodesEditorContext;

// Callback type used to specify special behavior when hovering a node in the
// minimap
...
void DestroyContext(
    ImNodesContext *ctx = NULL); // NULL = destroy current context
...
// NULL, the active context's ImNodesStyle instance will be used as the
// destination.
...
// Use PushColorStyle and PopColorStyle to modify ImNodesStyle::Colors
// mid-frame.
...
// If ImNodesStyleFlags_GridSnapping is enabled, snap the specified node's
// origin to the grid.
...
// Use the following functions to add or remove individual nodes or links from
// the current editors selection. Note that all functions require the id to be
// an existing valid id for this editor. Select-functions has the precondition
// that the object is currently considered unselected. Clear-functions has the
// precondition that the object is currently considered selected. Preconditions
// listed above can be checked via IsNodeSelected/IsLinkSelected if not already
// known.
...
bool IsLinkDropped(int *started_at_attribute_id = NULL,
...
bool IsLinkCreated(int *started_at_node_id, int *started_at_attribute_id,
                   int *ended_at_node_id, int *ended_at_attribute_id,
                   bool *created_from_snap = NULL);
...
const char *SaveEditorStateToIniString(const ImNodesEditorContext *editor,
                                       size_t *data_size = NULL);

void LoadCurrentEditorStateFromIniString(const char *data, size_t data_size);
void LoadEditorStateFromIniString(ImNodesEditorContext *editor,
                                  const char *data, size_t data_size);

void SaveCurrentEditorStateToIniFile(const char *file_name);
void SaveEditorStateToIniFile(const ImNodesEditorContext *editor,
                              const char *file_name);

void LoadCurrentEditorStateFromIniFile(const char *file_name);
void LoadEditorStateFromIniFile(ImNodesEditorContext *editor,
                                const char *file_name);
} // namespace IMNODES_NAMESPACE
