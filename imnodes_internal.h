...
struct ImNodeData {
  ...
  ImNodeData(const int node_id)
      : Id(node_id), Origin(0.0f, 0.0f), TitleBarContentRect(),
        Rect(ImVec2(0.0f, 0.0f), ImVec2(0.0f, 0.0f)), ColorStyle(),
        LayoutStyle(), PinIndices(), Draggable(true) {}
...
struct ImPinData {
  ...
  ImPinData(const int pin_id)
      : Id(pin_id), ParentNodeIdx(), AttributeRect(),
        Type(ImNodesAttributeType_None), Shape(ImNodesPinShape_CircleFilled),
        Pos(), Flags(ImNodesAttributeFlags_None), ColorStyle() {}
};
...
struct ImNodesEditorContext {
  ...
  ImNodesMiniMapNodeHoveringCallbackUserData
      MiniMapNodeHoveringCallbackUserData;
  ...
  ImNodesEditorContext()
      : Nodes(), Pins(), Links(), Panning(0.f, 0.f), SelectedNodeIndices(),
        SelectedLinkIndices(), SelectedNodeOffsets(),
        PrimaryNodeOffset(0.f, 0.f), ClickInteraction(), MiniMapEnabled(false),
...
struct ImNodesContext {
  ...
  // Event helper state
  // TODO: this should be a part of a state machine, and not a member of the
  // global struct. Unclear what parts of the code this relates to.
  int ImNodesUIState;
...
template <>
inline int ObjectPoolFindOrCreateIndex(ImObjectPool<ImNodeData> &nodes,
                                       const int node_id) {
